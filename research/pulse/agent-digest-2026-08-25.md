# What the machines noticed — 2026-08-25
...
5 product(s) we featured and can no longer price:
...
- designer: BROKEN RECORD — 4 finding(s) repeated three runs running (e.g. "medium::35 distinct colours"). Either they are not actionable or they are being ignored; the agent should say WHY instead of repeating itself.
...
- **YouTube** — the sealed market — "The whole sealed market in one number: 98.2."
- **TikTok** — Open on the number, not the setup: "The whole sealed market in one number: 98.2."

3 format gap(s): YouTube — No B-roll list ships with any angle.; TikTok — Every card we mint is 1200×675 — landscape.

## Something is slipping
- **unsaved** — 15 hand-written file(s) changed and not committed. *Generated artifacts churn constantly and that is fine. Hand-written work sitting uncommitted is a session's thinking one crash away from gone.*
- **ignored** — 8 supervisor problem(s) are open. *Problems that stay open stop being read. If they are not going to be fixed, they should be closed with a reason instead of carried.*
...
- **drift** — 20 script(s) are not referenced anywhere: artifact-freshness.mjs, backfill-artists.mjs, build-faq.mjs, build-hooks.mjs. *A script nothing calls is either dead or was wired up and quietly unwired. Both are worth knowing, and neither announces itself.*
...
## Design
1 high, 10 medium across 14 shipped surfaces, 44 minted cards.
...
42 open — 27 need eyes on a rendered page, 15 need a decision.
