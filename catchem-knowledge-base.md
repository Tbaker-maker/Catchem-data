...
### HOW WE WRITE
- THREE CARDS IS NOT TWO ENDS (Aug 26): boot loaded Moltres · Zapdos · Articuno and wrote "Moltres or Articuno. Which one is the post?" — the middle card vanished, and an `<input>` ate the line breaks so three sentences ran together. A 3-card painting names all three or it does not go in the box. The line field is a textarea.
- THE CAPTION FOLLOWS THE TRAY (Aug 26): searching Charizard after the beach left HYOGONOSUKE's sentence in the box. Name search never called fillLineFromCards. If the cards changed, the line changes.
- catchemtcg.com/build IS THE 68KB MOCKUP (Aug 26): GitHub Pages serves the 4MB editor. The public domain `/build` fetches card-index.json and has no Find, no connecting pins, no save sheet. Do not test the mockup and call the editor done.
...
- THE ACCOUNTS ARE SMALL (Aug 24): knoyhead 3k, xzuyyu 4.9k, CardGameNomad 9.5k, tall_alan 16k, Crambo 17.6k, shotgun 43k. Crambo's 37,100-view post came off 17.6k — 2.1x his follower count. Reach RELATIVE to size is the metric. CORRECTED 2026-08-25: this line cited 18,800 and asserted Tyler was already in this company without ever doing the division, and 18,800 was itself an unsettled mid-climb reading. @LongedEth has 21,554 followers (X API, 2026-08-25): 18,800 was 0.87x his count, BELOW Crambo and the opposite of what the line claimed; the settled 127,200 is 5.90x, or 2.8x Crambo's ratio. The law holds; the evidence cited for it did not. Caveat: the cross-account analysis asserts 2026 algorithm claims without sources — held at COMMUNITY confidence, a hypothesis to test rather than a rule.
...
- TIMING CONFOUNDED FORMAT (Aug 23) — WITHDRAWN 2026-08-25: the 122x never existed. One post was entered twice, and 9:18pm IS the 21:18 already recorded. The readings were 154 at 0.2h, 18,800 at 15.33h and a settled 127,200 at 59.8h — one post climbing, not two hours competing. I had logged 154 against the crop's 791 and concluded the crop shape won 5 to 1, drawing a conclusion about FORMAT from numbers mostly measuring HOUR, in the same session where I said the log needed 20 entries to mean anything. THE TWO-CARD PAIRING IS THE STRONGEST SHAPE WE HAVE. The hour is not optional metadata: any comparison between shapes that does not hold posting time roughly constant is measuring the time.
...
### HOW WE WORK
- A WRONG HOOK COSTS THEM, NOT US (Aug 25): the video hook generator's first draft divided by MISSING prices ("Tyranitar $0 to $4,250") and hit the prefix bug again. A creator reading that on camera gets corrected in their own comments. Every hook now carries a check field naming its counted basis; prices under $2 are UNKNOWN not free; ratios above 3000x are discarded as suspect.
- THE PREFIX BUG, FIXED PROPERLY (Aug 25): seventh appearance, "White Kyurem" became "White". Seven occurrences means the APPROACH was wrong, not the list — grouping now uses the DEX NUMBER, printed on the card and shared by every form, correct by construction and undefeatable by a prefix nobody has thought of.
- EVERY TEST I WRITE CHECKS SOMETHING I ALREADY IMAGINED (Aug 25): ten prompts I chose, fourteen abuses I thought of — none can find what I did not think of, which is why Tyler has found six things none caught. fuzz.mjs builds sentences from real vocabulary plus real noise, drives actions in an order nobody designed, and fails a quarter of runs on a blocked image host. FIRST RUN found three crashes (startStreak with an unknown filter, todaysCard and confirmPosted with no streak), then caught my own fix using the wrong parameter name. Every failure SIGNATURE is recorded with its exact input and replayed on every build — the value is not the crash, it is the shape of the crash recorded so it is never new twice.
