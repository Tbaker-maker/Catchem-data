...
- Runs via **GitHub Actions**. `workflow_dispatch` is on. Origin YAML still has `cron: "0 4 * * *"` — a PAT without `workflow` scope cannot delete it. Read the YAML, not this sentence.
