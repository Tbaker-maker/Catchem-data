...
:root{--bg:#0b0d14;--panel:#12141d;--line:#232736;--txt:#d8dde8;--dim:#8a93a8;--gold:#F5C842;--green:#36d399;--red:#ff6b7a}
*{box-sizing:border-box;margin:0}body{background:var(--bg);color:var(--txt);font:15px/1.5 'Trebuchet MS',system-ui,sans-serif;padding:0 0 60px}
...
.calib .fill{height:100%;width:${Math.min(100,Math.round(+heatDay/8*100))}%;background:linear-gradient(90deg,var(--gold),var(--green))}
.calib .lbl{font:12px 'Courier New',monospace;color:var(--dim);white-space:nowrap}
.stats{max-width:1080px;margin:10px auto 0;padding:0 24px;display:flex;gap:26px;font:13px 'Courier New',monospace;color:var(--dim)}
.stats b{color:var(--txt)}
main{max-width:1080px;margin:20px auto 0;padding:0 24px}
table{width:100%;border-collapse:collapse;background:var(--panel);border:1px solid var(--line);border-radius:10px;overflow:hidden}
th{font:11px 'Courier New',monospace;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);text-align:left;padding:12px 14px;border-bottom:1px solid var(--line)}
td{padding:11px 14px;border-bottom:1px solid var(--line);vertical-align:middle}
tr:last-child td{border-bottom:0}
.name{font-weight:600}.sub{display:block;font-size:11px;color:var(--dim);font-weight:400}
.pill{font:10px 'Courier New',monospace;border:1px solid var(--line);border-radius:20px;padding:3px 8px;color:var(--dim)}
.num{font:14px 'Courier New',monospace}
.spread{font:12px 'Courier New',monospace;border-radius:6px;padding:3px 8px}
...
footer{max-width:1080px;margin:22px auto 0;padding:0 24px;font:12px 'Courier New',monospace;color:var(--dim)}
</style></head><body>
