...
await writeFile(join(DATA,"supply-watch.json"), JSON.stringify({
  ...
  method: "7-day listing-count deltas, eBay vs TCG-side. Active Listings measured; demand est. from listing activity, not reported sales. Lead-lag states are reads. NOTE Aug 18: current provider exposes no sealed listing counts (tcgListings null) — TCG side dormant; eBay-side deltas remain valid via heat-history.",
