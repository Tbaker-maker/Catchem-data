...
const AGENTS = [
  ...
  { id: "decision-audit", zeroMeans: "good", output: "research/pulse/decision-audit.json", findings: d => (d?.due ?? []).map(x => `due::${x.id}`), maxFindings: 10, maxSilentDays: 7 },
  { id: "designer", zeroMeans: "suspect", output: "research/pulse/design-audit.json", findings: d => (d?.findings ?? []).map(f => `${f.severity}::${String(f.what).slice(0,40)}`), maxFindings: 20, maxSilentDays: 7 },
  { id: "theme-scout", zeroMeans: "suspect", output: "research/pulse/theme-scout.json", findings: d => (d?.finds ?? []).map(f => `${f.kind}::${String(f.headline).slice(0,40)}`), maxFindings: 30, maxSilentDays: 7 },
  { id: "teacher", zeroMeans: "good", output: "research/pulse/teacher.json", findings: d => (d?.lessons ?? []).map(l => `${l.kind}::${l.agent}`), maxFindings: 14, maxSilentDays: 7 },
  { id: "api-strategist", zeroMeans: "suspect", output: "research/pulse/api-strategy.json", findings: d => (d?.findings ?? []).map(f => `${f.severity}::${String(f.what).slice(0,40)}`), maxFindings: 12, maxSilentDays: 7, standing: true },
...
const dispatch = { chat: [], cc: [], tyler: [] };
{
  const ROUTE = {
    ...
    "decision-audit": { file: "research/pulse/decision-audit.json", pick: d => (d?.due ?? []).map(x => ({ what: `a prediction has come due: ${x.decision}`, do: `Predicted: ${x.predicted} — did it hold?`, owner: "tyler" })) },
    designer: { file: "research/pulse/design-audit.json", pick: d => (d?.findings ?? []).filter(f => f.severity === "high").map(f => ({ what: `${f.surface}: ${f.what}`, do: f.fix, owner: "chat" })) },
    "theme-scout": { file: "research/pulse/theme-scout.json", pick: d => (d?.finds ?? []).filter(f => f.needsHuman).map(f => ({ what: f.headline, do: f.why, owner: "tyler" })) },
    teacher: { file: "research/pulse/teacher.json", pick: d => (d?.lessons ?? []).filter(l => l.kind === "post-mortem").map(l => ({ what: `${l.agent}: ${l.observation}`, do: l.question, owner: "chat" })) },
    "api-strategist": { file: "research/pulse/api-strategy.json", pick: d => (d?.findings ?? []).map(f => ({ what: f.what, do: f.action, owner: f.owner })) },
