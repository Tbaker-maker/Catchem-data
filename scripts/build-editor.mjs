...
<label class="howmany" id="artflipwrap"><span>Art flip</span>
  <select id="artflip" aria-label="Art flip">
    <option value="0" selected>Card only</option>
    <option value="1">Card and the art</option>
  </select>
</label>
...
// ART FLIP is paper TCG only: each card is followed by a crop of its
// illustration. Quantity still means cards; the clip is extra. Connecting
// art is already the picture, so it is never doubled.
var artFlip = false;
function artFlipOn(){
  return !!artFlip && currentGame() === "paper";
}
...
function applyArtFlip(on, rerun){
  artFlip = !!on;
  var sel = el("artflip");
  if (sel && String(sel.value) !== (artFlip ? "1" : "0")) sel.value = artFlip ? "1" : "0";
  try { paintArtFlip(); } catch (e0) {}
  try { paintCountOptions(); } catch (e1) {}
  if (document.body && document.body.setAttribute)
    document.body.setAttribute("data-artflip", artFlipOn() ? "on" : "off");
...
function categoryMax(g){
  ...
  m = m > 0 ? m : 9;
  // Art flip doubles each card into a pair. Four cards is eight
  // slots — the last page that still has a frame. Connecting art
  // never uses this cap.
...
window.setMode = setMode;
{
  ...
  if (artSel) artSel.onchange = function(){ applyArtFlip(artSel.value === "1", true); };
...
function render(){
  ...
  const pocketArt = function(c){
    if (!c) return "<div class='pocket'></div>";
    return '<div class="pocket filled artclip"><img src="' + imgSmall(c.i) + '" alt="' + String(c.n).replace(/"/g, "") + ' art" loading="eager" data-cid="' + c.i + '" onerror="imgFallback(this,&#39;' + c.i + '&#39;)"></div>';
  };
  let html = "";
  if (L && L.shape && L.shape.length) {
    var pi = 0;
  ...
  if (L) {
    ...
    var extra = (typeof artFlipActive === "function" && artFlipActive()) ? " · each with its art" : "";
...
async function composeImage(){
  ...
  const CAP = connecting || (typeof artFlipActive === "function" && artFlipActive()) ? 0 : (tray.length <= 4 ? 70 : 0);
