...
.pocket.artclip > img{
  position:absolute;inset:auto;width:245%;height:245%;max-width:none;
  left:50%;top:-29%;transform:translateX(-50%);
...
function applyArtCrop(mode, rerun){
  ...
  if (rerun && lastPref && lastPref.ask) {
    runAsk(lastPref.ask);
    return;
  }
...
// Illustration window of a printed TCG card, then cover-fit into the
// destination slot. Same image as the card — never a second URL.
function artSrcRect(img, dw, dh){
  var iw = img.naturalWidth || img.width || 1;
  var ih = img.naturalHeight || img.height || 1;
  var ax = iw * 0.075, ay = ih * 0.12, aw = iw * 0.85, ah = ih * 0.41;
  var dest = dw / dh, src = aw / ah;
  var sx = ax, sy = ay, sw = aw, sh = ah;
  if (src > dest) {
    sw = ah * dest;
    sx = ax + (aw - sw) / 2;
  } else {
...
function drawArtClip(g, img, x, y, cw, ch){
  var r = artSrcRect(img, cw, ch);
  try {
    g.drawImage(img, r.sx, r.sy, r.sw, r.sh, x, y, cw, ch);
  } catch (e) {
    g.drawImage(img, x, y, cw, ch);
  }
}
...
function render(){
  ...
  const pocketArt = function(c, k){
    ...
    return '<div class="pocket filled artclip"><img src="' + imgSmall(c.i) + '" alt="' + String(c.n).replace(/"/g, "") + ' art" loading="eager" data-cid="' + c.i + '" onerror="imgFallback(this,&#39;' + c.i + '&#39;)">' + buttons + "</div>";
  };
  let html = "";
  if (L && L.shape && L.shape.length) {
    var pi = 0;
...
async function composeImage(){
  ...
  try{
    ...
    for (let i=0;i<slotList.length;i++){
      ...
      if (slotList[i].art) drawArtClip(g, img, x, y, CW, CH);
