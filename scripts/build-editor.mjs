...
function renderThemes(){
  ...
  const canFill = (t) => {
    ...
    if (t.id === "many-hands" || t.id === "battle") {
      const byMon = {};
      for (const c of pool) if (c.a) (byMon[monName(c.n)] ||= new Set()).add(c.a);
...
function buildIdeas(){
  ...
  if (shape === "list") {
    ...
    const byName = {};
    for (const c of pool) if (c.a) (byName[monName(c.n)] ||= []).push(c);
    ...
    const byMon = {};
    for (const c of pool) if (c.p && c.a) (byMon[monName(c.n)] ||= []).push(c);
    ...
    const byMon = {};
    for (const c of pool) if (c.y) (byMon[monName(c.n)] ||= []).push(c);
    for (const [mon, list] of Object.entries(byMon)) {
    ...
    const byMon = {};
    for (const c of withHp) (byMon[monName(c.n)] ||= []).push(c);
    for (const [mon, list] of Object.entries(byMon)) {
      const sorted = list.sort((a, b) => (a.y || "") < (b.y || "") ? -1 : 1);
