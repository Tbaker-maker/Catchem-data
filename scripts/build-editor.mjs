...
function runAsk(text){
  ...
  if (!reply.ok) return;
  // Reset before applying, or two sentences in a row compound into nonsense.
  // CLEAR THE TRAY TOO. Resetting the filters but keeping the previous tray
  // means a prompt that finds nothing inherits the LAST answer and reports it as
  // this one — which is how "pikachu cards" returned Charmander.
  fMon = null; fRating = null; fSet = ""; fTheme = null;
  tray = []; blob = null; window.__ideas = [];
  if (found.count) fCount = found.count;
  // AN EVOLUTION IS THREE CARDS unless they said otherwise. Defaulting to two
  // gives half a line and calls it the line.
  else if (found.shape === "evo-line") fCount = 3;
  ...
  // worked, so it gets a direct path rather than an approximation.
  if (found.shape === "obscure") {
    const counts = {};
    for (const c of INDEX) { const m = monName(c.n); counts[m] = (counts[m] || 0) + 1; }
    const FAMOUS = /Charizard|Pikachu|Eevee|Umbreon|Mewtwo|Rayquaza|Lugia|Gengar|Blastoise|Venusaur/i;
    const pool = INDEX.filter(c => /Illustration Rare/i.test(c.r || "") && c.a && !FAMOUS.test(c.n) && counts[monName(c.n)] <= 8 && (c.p || 0) >= 2);
    const seen = new Set(); const picked = [];
    for (const c of pool.sort(function(a,b){ return counts[monName(a.n)] - counts[monName(b.n)]; })) {
      const k = monName(c.n);
      if (seen.has(k)) continue;
      seen.add(k); picked.push(c);
      if (picked.length >= (fCount || 4)) break;
    }
    if (picked.length) { tray = picked; blob = null; el("label").value = "Obscure is the point."; render(); return; }
  }
  resetPage(); search(); renderThemes(); buildIdeas();

  // THE BOX PROMISES A POST, SO HAND BACK CARDS. Setting filters and calling
  // search() only changes the browse grid — seven prompts out of eleven left the
  // tray empty, so the user asked for a post and got a list to click through.
  // If a theme produced ideas, load the first one. If not, take the best cards
  // the filters allow.
  const ideas = window.__ideas || [];
  if (ideas.length && ideas[0].cards && ideas[0].cards.length) {
    tray = ideas[0].cards.slice(0, fCount || 2);
    blob = null;
    if (ideas[0].hook && el("label") && !el("label").value.trim()) el("label").value = ideas[0].hook;
    render();
    return;
  }
  // NO THEME FITTED. Fall back to the filtered pool rather than nothing — one
  // card per Pokémon, best first, because nine Charizards is a composition and
  // not a set of nine.
  const pool = INDEX.filter(function(c){
    return monPass(c) && ratingPass(c) && HERO_RX.test(c.r || "") && c.a && (!fSet || c.s === fSet);
  });
  const byMon = {};
  for (const c of pool) { const k = monName(c.n);
    if (!byMon[k] || (c.p || 0) > (byMon[k].p || 0)) byMon[k] = c; }
  const picked = Object.values(byMon).sort(function(a, b){ return (b.p || 0) - (a.p || 0); }).slice(0, fCount || 2);
  if (picked.length) { tray = picked; blob = null; render(); }
  else {
    // SAY SO RATHER THAN LEAVING IT BLANK. An empty tray with no explanation is
    // indistinguishable from broken, which is what this whole fix is about.
    const box = el("askreply");
...
function buildIdeas(){
  ...
  if (shape === "list") {
    ...
    for (const [base, card] of Object.entries(byMon)) {
      ...
      ideas.push({ title: line.map(c => monName(c.n)).join(" → "),
