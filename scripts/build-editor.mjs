...
#artflipwrap{margin-top:-8px}
body[data-visual="pocket"] #artflipwrap,
body[data-visual="both"] #artflipwrap,
body[data-visual="movies"] #artflipwrap,
body[data-visual="consoles"] #artflipwrap,
body[data-visual="cartridges"] #artflipwrap{display:none}
...
.pocket.artclip{background:#070910}
.pocket.artclip > img.artbg{
  position:absolute;inset:-18%;width:136%;height:136%;max-width:none;
  object-fit:cover;object-position:50% 28%;
  filter:blur(18px) saturate(1.15);opacity:.8;outline:none}
.pocket.artclip > img.artfg{
  position:absolute;left:-9%;top:12%;width:118%;height:118%;max-width:none;
  object-fit:fill;outline:none}
.pocket.artclip.fullart > img.artfg{
  left:-6%;top:1%;width:112%;height:112%}
...
<label class="howmany" id="artflipwrap"><span>Art crop</span>
  <select id="artflip" aria-label="Art crop">
    <option value="0" selected>Full card</option>
...
function layoutForTray(){
  ...
  var n = tray.length;
  if (typeof artFlipActive === "function" && artFlipActive()) {
    if (n === 1) return LAYOUTS[2];
    if (n === 2) return LAYOUTS[4];
    if (n === 3) return artPairFrame(3);
    if (n === 4) return LAYOUTS[8];
    return null;
  }
  return LAYOUTS[n] || null;
}
// ART CROP is paper TCG only. Full card is the default. Just art
// replaces each card with a crop of its illustration. Both sits
// the crop next to the card. Quantity still means cards; Both
// doubles the slots so it stops at four. Connecting art is
// already the picture, so it is never cropped or doubled.
var artCrop = "card";
function artCropMode(){ return artCrop; }
function artCropValue(){
  if (artCrop === "art") return "art";
  if (artCrop === "both") return "1";
  return "0";
}
function parseArtCrop(v){
  if (v === true || v === 1 || v === "1" || v === "both") return "both";
  if (v === "art" || v === "just") return "art";
  return "card";
}
function artFlipOn(){
  return artCrop === "both" && currentGame() === "paper";
}
function artJustOn(){
  return artCrop === "art" && currentGame() === "paper";
}
function artFlipActive(){
  if (!artFlipOn()) return false;
  if (!tray || !tray.length) return false;
  if (connectingGroupOf(tray)) return false;
  return true;
}
function artJustActive(){
  if (!artJustOn()) return false;
  if (!tray || !tray.length) return false;
  if (connectingGroupOf(tray)) return false;
  return true;
}
function artPairFrame(rows){
  var CW = 745, CH = 1040, PAD = 90, GAP = 60;
  var W = PAD * 2 + CW * 2 + GAP;
  var H = PAD * 2 + rows * CH + (rows - 1) * GAP;
  return { name: "the pairing page", cols: 2, rows: rows, cardW: CW, cardCaption: 0,
    W: W, H: H, ratio: H / W, artFlip: true, cropExempt: true };
}
function composeSlots(){
  if (artFlipActive()) {
    var out = [];
    for (var i = 0; i < tray.length; i++) {
      out.push({ card: tray[i], art: false, trayIndex: i });
      out.push({ card: tray[i], art: true, trayIndex: i });
    }
    return out;
  }
  if (artJustActive()) {
    return (tray || []).map(function(c, i){ return { card: c, art: true, trayIndex: i }; });
  }
  return (tray || []).map(function(c, i){ return { card: c, art: false, trayIndex: i }; });
}
function applyArtFlip(on, rerun){
  applyArtCrop(parseArtCrop(on), rerun);
}
function applyArtCrop(mode, rerun){
  var prev = artCrop;
  artCrop = parseArtCrop(mode);
  var sel = el("artflip");
  var want = artCropValue();
  if (sel && String(sel.value) !== want) sel.value = want;
  try { paintArtFlip(); } catch (e0) {}
  try { paintCountOptions(); } catch (e1) {}
  if (document.body && document.body.setAttribute)
    document.body.setAttribute("data-artcrop", artCrop);
  if (artCrop !== prev) blob = null;
  if (artFlipOn() && tray && tray.length > 4 && !connectingGroupOf(tray)) {
    tray = tray.slice(0, 4);
    blob = null;
  }
  if (rerun && lastPref && lastPref.ask && typeof countIsFit === "function" && countIsFit() && (artCrop === "both" || prev === "both") && artCrop !== prev) {
    runAsk(lastPref.ask);
    return;
  }
  render();
}
function paintArtFlip(){
  var wrap = el("artflipwrap");
  var paper = currentGame() === "paper";
  if (wrap) {
    wrap.hidden = !paper;
    if (wrap.style) wrap.style.display = paper ? "" : "none";
  }
  var sel = el("artflip");
  var want = artCropValue();
  if (sel && String(sel.value) !== want) sel.value = want;
}
window.artFlipOn = artFlipOn;
window.artJustOn = artJustOn;
window.artJustActive = artJustActive;
window.artCropMode = artCropMode;
window.applyArtFlip = applyArtFlip;
window.applyArtCrop = applyArtCrop;
window.composeSlots = composeSlots;
window.artWindow = artWindow;
window.isFullArtCard = isFullArtCard;
...
function parseIntent(text, ctx) {
  ...
  if (num && [1, 2, 3, 4, 6, 8, 9].includes(Number(num[1]))) { found.count = Number(num[1]); found.matched.push(found.count + " cards"); }
    ...
    if (w === "one" && /one painting|one picture|one beach|one diamond|one star|one shiny|first one/.test(q)) continue;
...
// Illustration window of a printed TCG card. Regular cards have a landscape
// painting under the name. LV.X / V / full art IS the painting, so the
// window is taller. Cover-fitting a landscape window into a portrait slot
// was the body-close-up bug: we now draw the whole window, contained, on
// a blurred fill of itself.
function isFullArtCard(card){
  var n = String((card && card.n) || "");
  var r = String((card && card.r) || "").toLowerCase();
  if (n.indexOf("LV.X") >= 0 || n.indexOf("LVX") >= 0) return true;
  if (n.indexOf("VMAX") >= 0 || n.indexOf("VSTAR") >= 0) return true;
  if (n.slice(-3) === " GX" || n.slice(-2) === " V" || n.slice(-3) === " ex" || n.slice(-3) === " EX") return true;
  if (r.indexOf("illustration") >= 0 || r.indexOf("full art") >= 0 || r.indexOf("alternate") >= 0) return true;
  return false;
}
function artWindow(img, card){
  var iw = img.naturalWidth || img.width || 1;
  var ih = img.naturalHeight || img.height || 1;
  if (isFullArtCard(card)) {
    return { sx: iw * 0.05, sy: ih * 0.085, sw: iw * 0.90, sh: ih * 0.70 };
  }
  return { sx: iw * 0.075, sy: ih * 0.125, sw: iw * 0.85, sh: ih * 0.40 };
}
function artSrcRect(img, dw, dh, card){
  return artWindow(img, card);
}
function coverArt(g, img, r, x, y, w, h){
  var dest = w / h, src = r.sw / r.sh;
...
function containArt(g, img, r, x, y, w, h){
  var scale = Math.min(w / r.sw, h / r.sh);
  var dw = r.sw * scale, dh = r.sh * scale;
  var dx = x + (w - dw) / 2, dy = y + (h - dh) / 2;
  g.drawImage(img, r.sx, r.sy, r.sw, r.sh, dx, dy, dw, dh);
}
function drawArtClip(g, img, x, y, cw, ch, card){
  var r = artWindow(img, card);
  g.save();
  roundRectSafe(g, x, y, cw, ch, cw * 0.045);
  g.clip();
  g.fillStyle = "#070910";
  g.fillRect(x, y, cw, ch);
  try {
    containArt(g, img, r, x + cw * 0.04, y + ch * 0.08, cw * 0.92, ch * 0.84);
  } catch (e) {
    try { g.drawImage(img, x, y, cw, ch); }
    catch (e2) {}
  }
  g.restore();
}
function paintArtClips(){
  var box = el("tray");
  if (!box || !box.querySelectorAll) return;
  var nodes = box.querySelectorAll("canvas.artcv");
  for (var i = 0; i < nodes.length; i++) {
    (function(cv){
      var wrap = cv.parentNode;
      var id = wrap && wrap.getAttribute ? wrap.getAttribute("data-art") : "";
      var card = (typeof byIdRow !== "undefined" && id && byIdRow[id]) ? byIdRow[id] : null;
      var im = new Image();
      im.onload = function(){
        var g = cv.getContext("2d");
        if (!g) return;
        g.clearRect(0, 0, cv.width, cv.height);
        drawArtClip(g, im, 0, 0, cv.width, cv.height, card);
      };
      im.src = imgSmall(id);
    })(nodes[i]);
  }
...
function currentGame(){
  ...
  if (INDEX.length && INDEX[0]) {
    if (INDEX[0].g === "k") return "pocket";
    if (INDEX[0].g === "m") return "movies";
    if (INDEX[0].g === "h") return "consoles";
    if (INDEX[0].g === "t") return "cartridges";
  }
  return "paper";
}
...
function askArtistLoose(text, monName){
  ...
  for (var k in ASK_ARTISTS) {
    ...
    for (var j = 0; j < parts.length; j++) {
      ...
      // "game boy" is a handheld, not GAME FREAK inc. "nintendo 64" is a
      // console, not a credit that happens to start with Nintendo.
      if (p === "game" || p === "nintendo" || p === "inc") continue;
...
function categoryMax(g){
  var copy = categoryCopy(g);
  var m = Number(copy && copy.max);
  m = m > 0 ? m : 9;
  // Art crop "Both" doubles each card into a pair. Four cards is eight
  // slots — the last page that still has a frame. Just art keeps the
  // usual page. Connecting art never uses this cap.
  if (typeof artFlipOn === "function" && artFlipOn()) {
    if (m > 4) m = 4;
  }
  return m;
}
function takeCap(){
  ...
  if (locked) return locked > max ? max : locked;
  return max;
}
...
function pocketShowcase(){
  if (!POCKET_INDEX || !POCKET_INDEX.length) return false;
  ...
  for (var i = 0; i < POCKET_INDEX.length; i++) {
    var c = POCKET_INDEX[i];
    if (c.sup !== "P") continue;
    if (!c.hero && !/Three Star|Crown|Two Star/i.test(c.r || "")) continue;
    var sid = c.sid || c.s || "?";
    if (!buckets[sid]) { buckets[sid] = []; order.push(sid); }
    buckets[sid].push(c);
  }
  for (var s in buckets) buckets[s].sort(function(a, b){ return pocketScore(b) - pocketScore(a); });
  var pick = [], guard = 0;
  while (pick.length < nNeed && guard < 80) {
    guard++;
    var progressed = false;
    for (var oi = 0; oi < order.length && pick.length < nNeed; oi++) {
      var list = buckets[order[oi]];
      if (list && list.length) { pick.push(list.shift()); progressed = true; }
    }
    if (!progressed) break;
  }
  ...
  tray = pick;
  blob = null;
  lastPref = { kind: "ask", ask: "pokemon pocket" };
  anotherCursor = 0;
  var box = el("askreply");
  if (box) {
    box.textContent = "Pokémon Pocket — " + POCKET_INDEX.length.toLocaleString("en-US") + " cards. A pull from across the sets.";
...
function movieShowcase(){
  ...
  if (pick.length < cap) {
    for (var j = 0; j < MOVIE_INDEX.length && pick.length < cap; j++) {
      if (pick.indexOf(MOVIE_INDEX[j]) < 0) pick.push(MOVIE_INDEX[j]);
    }
  }
  tray = pick.slice(0, cap);
  blob = null;
...
function visualShowcase(index, prefer, label){
  ...
  for (var j = 0; j < index.length && pick.length < cap; j++) {
...
function visualAsk(index, t, whyEmpty){
  ...
  if (/ handhelds /.test(t)) {
    ...
    if (hands.length)
      return { relation: "HARDWARE_PIN", pin: hands.slice(0, takeCap()).map(function(c){ return c.i; }).join("|"), why: "the handhelds", need: Math.min(takeCap(), hands.length) };
  }
  ...
  for (var i = 0; i < index.length; i++) {
    var c = index[i];
    var sc = 0;
    var keys = (c.aliases || []).concat(c.scenes || []).concat([c.n]);
...
function visualHome(t){
  ...
  var order = cur === "consoles" ? ["cartridges", "movies"]
    ...
    : ["consoles", "cartridges"];
  return visualHomeOrder(t, order);
}
function visualHomeAny(t){
  // Paper miss: consoles first, so "game boy" is the handheld, not a box
  // whose platform field says Game Boy.
  return visualHomeOrder(t, ["consoles", "cartridges", "movies"]);
}
function visualHomeOrder(t, order){
...
function tryVisualHomeAsk(text){
  var gNow = currentGame();
  if (gNow === "movies" || gNow === "consoles" || gNow === "cartridges") return false;
  var t = " " + String(text || "").toLowerCase().replace(/[^a-z0-9]+/g, " ") + " ";
  var home = visualHomeAny(t);
  if (!home) return false;
  applyGame(home.g, false);
  var got = askCards(home.r);
  if (!got.cards || !got.cards.length) return false;
  var box0 = el("askreply");
  tray = got.cards.slice(0, takeCap());
  blob = null;
  lastPref = { kind: "ask", ask: text };
  anotherCursor = 0;
  if (box0) {
    box0.textContent = visualFactReply(got.cards, home.r.why);
...
// VISUAL CONTENT, THEN THE CATEGORY. The picker is the umbrella. The lede,
// the count label and the page label name the thing you actually picked, so
...
function paintCountOptions(g){
  var copy = categoryCopy(g);
  var max = 9;
  try { max = categoryMax(g); } catch (eM) { max = Number(copy.max) || 9; }
  var sel = el("cardcount");
  if (sel && sel.options) {
    var labels = {};
    labels["0"] = copy.fit;
    ...
    labels["9"] = "9 — " + (copy.many || "a binder page");
    if (typeof artFlipOn === "function" && artFlipOn()) {
      labels["1"] = "1 — one card and its art";
      labels["2"] = "2 — two pairs";
      labels["3"] = "3 — three pairs";
      labels["4"] = "4 — four pairs";
    } else if (labels[String(max)]) {
      labels[String(max)] = max + " — " + (copy.many || labels[String(max)].split(" — ")[1] || "");
    }
    for (var i = 0; i < sel.options.length; i++) {
      var opt = sel.options[i];
      var v = String(opt.value);
      var n = Number(v);
      var on = v === "0" || (FORMAT_N.indexOf(n) >= 0 && n <= max);
      if (labels[v]) {
        opt.text = labels[v];
      ...
      opt.disabled = !on;
      opt.hidden = !on;
      if (opt.style) opt.style.display = on ? "" : "none";
    }
    if (Number(sel.value) > max) {
      try { markCount(0); } catch (e) {}
    }
  }
  ...
  if (chips && chips.querySelectorAll) {
    chips.querySelectorAll("[data-n]").forEach(function(x){
      var n = Number(x.getAttribute("data-n") || x.dataset.n);
      var on = FORMAT_N.indexOf(n) >= 0 && n <= max;
      x.hidden = !on;
      if (x.style) x.style.display = on ? "" : "none";
    });
  }
}
function paintCategory(mode, g){
  ...
  if (!mode) {
    mode = (document.body && document.body.getAttribute) ? (document.body.getAttribute("data-mode") || "post") : "post";
  }
  var l = el("pagelede");
  if (l) l.textContent = mode === "reply" ? copy.reply : copy.lede;
  var cl = el("countlabel");
  if (cl) cl.textContent = copy.count;
  var cs = el("cardcount");
  if (cs && cs.setAttribute) cs.setAttribute("aria-label", copy.aria || copy.count);
  if (document.body && document.body.setAttribute) document.body.setAttribute("data-visual", g || currentGame());
  try { paintCountOptions(g || currentGame()); } catch (e) {}
  try { paintArtFlip(); } catch (e2) {}
}
...
function applyGame(g, rerun){
  ...
  try { paintCategory(null, g); } catch (e) {}
  try { paintExamples(); } catch (e) {}
  try { renderStreak(); } catch (e) {}
  try { resetPage(); search(); } catch (e) {}
...
function setGame(g){
  ...
  var onTray = tray || [];
  var pocketN = 0;
  for (var i = 0; i < onTray.length; i++)
    if (onTray[i].g === "k" || String(onTray[i].i).indexOf("tcgp-") === 0) pocketN++;
  if (g === "pocket" && pocketN < 2) pocketShowcase();
}
...
function askCards(r, opts){
  ...
  if (r.relation === "MOVIE_PIN" || r.relation === "HARDWARE_PIN") {
    ...
    var capPin = 9;
    try { capPin = takeCap(); } catch (ePin) {}
    if (capPin < 1) capPin = 9;
    if (r.show || !r.pin) {
      if (currentGame() === "consoles")
        return { cards: CONSOLE_INDEX.slice(0, capPin), reason: r.why || "consoles" };
      if (currentGame() === "cartridges")
        return { cards: CART_INDEX.slice(0, capPin), reason: r.why || "cartridges" };
      var show = [];
      var prefer = ["mov-m01-poster","mov-m02-poster","mov-m05-poster","mov-m08-poster","mov-m10-poster","mov-m20-poster","mov-m23-poster","mov-mdp-poster","mov-m13-poster"];
      for (var mi = 0; mi < prefer.length && show.length < capPin; mi++) if (byId(prefer[mi])) show.push(byId(prefer[mi]));
      return { cards: show.length ? show : MOVIE_INDEX.slice(0, capPin), reason: r.why || "movie posters" };
    }
    var pinnedM = r.pin.split("|").map(byId).filter(Boolean);
    return { cards: pinnedM.slice(0, capPin), reason: r.why || "poster" };
  }
  ...
  if (r.relation === "SURPRISE") {
    ...
    if (cg === "movies" || cg === "consoles" || cg === "cartridges") {
      ...
      try { var maxV = categoryMax(); if (nNeedV > maxV) nNeedV = maxV; } catch (eV) { if (nNeedV > 9) nNeedV = 9; }
...
function pickCaption(){
  ...
  if (tray && tray.length && tray[0] && isVisual(tray[0])) {
    // THE IMAGE IS THE POST. Dumping year · theatrical poster · English
    // title · species onto the canvas at 52px ran "The Power of One 1999 ·
    // poste" off both edges and printed "artist no" in the footer. A
    // Pokémon Center post is the art, then the name, then the year.
    if (tray.length === 1) {
      var v = tray[0];
      return v.n + (v.y ? " · " + v.y : "");
    }
    var names = [];
    for (var vi = 0; vi < tray.length; vi++) names.push(tray[vi].n);
...
function runAsk(text){
  ...
  var saysConsole = tl.indexOf(" console ") >= 0 || tl.indexOf(" consoles ") >= 0
    ...
    || tl.indexOf(" game boy ") >= 0 || tl.indexOf(" gameboy ") >= 0
    || tl.indexOf(" nintendo 64 ") >= 0 || tl.indexOf(" n64 ") >= 0
    || tl.indexOf(" gamecube ") >= 0 || tl.indexOf(" game cube ") >= 0
    || tl.indexOf(" nintendo switch ") >= 0
    || tl.indexOf(" nintendo ds ") >= 0 || tl.indexOf(" nintendo 3ds ") >= 0
    || tl.indexOf(" pokewalker ") >= 0 || tl.indexOf(" poke walker ") >= 0;
  var saysGbp = tl.indexOf(" game boy pocket ") >= 0 || tl.indexOf(" pocket game boy ") >= 0;
  var saysCart = tl.indexOf(" cartridge ") >= 0 || tl.indexOf(" cartridges ") >= 0
    || tl.indexOf(" cart ") >= 0 || tl.indexOf(" carts ") >= 0;
  if (saysGbp && currentGame() !== "consoles") applyGame("consoles", false);
  ...
  var saysPocket = !saysGbp && (tl.indexOf(" pokemon pocket ") >= 0 || tl.indexOf(" tcg pocket ") >= 0
  ...
  const tryRelation = function(){
    ...
    if (got.cards.length > 1 || ((askRel.relation === "MOVIE_PIN" || askRel.relation === "HARDWARE_PIN" || askRel.relation === "SURPRISE") && got.cards.length >= 1)) {
      ...
      var lockedN = countLocked();
      // FIT (0) IS NOT A COUNT. The ask picks how many, then takeCap is the
      // catalogue ceiling — posters stop at 6, murals at 4, games at 9.
      // Connecting art is one picture and is never cropped to a cap.
      if (askRel.relation === "MOVIE_PIN" || askRel.relation === "HARDWARE_PIN")
        tray = got.cards.slice(0, takeCap());
      else if (askRel.relation === "CONNECTING_ART")
        tray = got.cards.slice(0, 9);
      else if (askRel.relation === "SURPRISE")
        tray = got.cards.slice(0, takeCap());
      else if (askRel.relation === "SET_FACTS")
        tray = got.cards.slice(0, takeCap());
      else if (lockedN) tray = got.cards.slice(0, takeCap());
      else tray = got.cards.slice(0, takeCap());
      blob = null;
      lastPref = { kind: "ask", ask: text };
      anotherCursor = 0;
  ...
  if (!reply.ok && tryVisualHomeAsk(text)) return;
  if (!reply.ok) {
    tray = []; blob = null;
    try { closeSaveSheet(); } catch (e) {}
  ...
  if (!res.cards.length && tryVisualHomeAsk(text)) return;
  if (res.cards.length) {
...
window.setMode = setMode;
{
  ...
  const artSel = el("artflip");
  if (artSel) artSel.onchange = function(){ applyArtCrop(artSel.value, true); };
...
function rerunAsk(text, exclude, rot){
  ...
  var capR = 9;
  try { capR = takeCap(); } catch (eR) {}
  // Connecting art is one picture. Never crop it to a poster wall.
  var sliceN = (rel && rel.relation === "CONNECTING_ART") ? 9 : capR;
  if (rel && (rel.relation === "SPECIES_GROUP" || rel.relation === "CONNECTING_ART"
    ...
    if (got.cards && got.cards.length > 1) return { cards: got.cards.slice(0, sliceN), why: [got.reason] };
  }
  const ctx = intentCtx();
  const found = parseIntent(text, ctx);
  const reply = intentReply(found, ctx);
  ...
  var facts = factSearch(text, capR).filter(function(c){ return !exclude.has(c.i); });
...
function render(){
  ...
  const pocket = (c, k) => {
    ...
    var capBits = [];
    if (c.y) capBits.push(String(c.y));
    var kk = kindLabel(c);
    if (kk) capBits.push(kk);
    var cap = vis ? '<span class="vcap"><b>' + String(c.n).replace(/</g, "") + '</b><i>' + capBits.join(" · ") + '</i></span>' : "";
    return '<div class="' + kls + '"><img src="' + imgSmall(c.i) + '" alt="' + String(c.n).replace(/"/g, "") + '" loading="eager" data-cid="' + c.i + '" onerror="imgFallback(this,&#39;' + c.i + '&#39;)"><button class="swap" onclick="swapSlot(' + k + ')" aria-label="Swap ' + String(c.n).replace(/"/g, "") + '">↻</button><button class="x" onclick="remove(' + k + ')" aria-label="Remove ' + String(c.n).replace(/"/g, "") + '">×</button>' + own + cap + "</div>";
  };
  const pocketArt = function(c, k){
    if (!c) return "<div class='pocket'></div>";
    var full = isFullArtCard(c) ? " fullart" : "";
    var buttons = (k == null || k === undefined) ? "" :
      '<button class="swap" onclick="swapSlot(' + k + ')" aria-label="Swap ' + String(c.n).replace(/"/g, "") + '">↻</button><button class="x" onclick="remove(' + k + ')" aria-label="Remove ' + String(c.n).replace(/"/g, "") + '">×</button>';
    return '<div class="pocket filled artclip' + full + '" data-art="' + c.i + '"><img class="artbg" src="' + imgSmall(c.i) + '" alt="" aria-hidden="true"><img class="artfg" src="' + imgSmall(c.i) + '" alt="' + String(c.n).replace(/"/g, "") + ' art" loading="eager" data-cid="' + c.i + '" onerror="imgFallback(this,&#39;' + c.i + '&#39;)">' + buttons + "</div>";
  };
  let html = "";
  if (L && L.shape && L.shape.length) {
    var pi = 0;
  ...
  } else if (typeof artFlipActive === "function" && artFlipActive()) {
    var cells = [];
    for (var ti = 0; ti < tray.length; ti++) {
      cells.push(pocket(tray[ti], ti));
      cells.push(pocketArt(tray[ti]));
    }
    html = cells.join("");
    var slotN = L ? L.cols * Math.ceil(cells.length / L.cols) : cells.length;
    for (var si = cells.length; si < slotN; si++) html += "<div class='pocket'></div>";
  } else if (typeof artJustActive === "function" && artJustActive()) {
    html = tray.map(function(c, k){ return pocketArt(c, k); }).join("");
    var justSlots = L ? L.cols * Math.ceil(tray.length / L.cols) : Math.max(tray.length, 3);
    for (var ji = tray.length; ji < justSlots; ji++) html += "<div class='pocket'></div>";
  } else {
  ...
  try { paintArtClips(); } catch (ePaint) {}
  ...
  renderTally();
  renderItemFacts();
  el("plabel").textContent = L ? (categoryPageLabel() + " — " + L.name.toUpperCase()) : categoryPageLabel();
  ...
  if (L) {
    ...
    var extra = "";
    if (typeof artFlipActive === "function" && artFlipActive()) extra = " · each with its art";
    else if (typeof artJustActive === "function" && artJustActive()) extra = " · just the art";
    setStatus(tray.length + (tray.length === 1 ? " card" : " cards") + " · " + L.cols + " across" + extra + (missing ? " · " + missing + " will publish with NO artist credit" : ""), missing > 0);
  } else {
...
function fitLine(g, text, maxW){
  ...
  var ell = "…";
  var t = text;
  while (t.length > 1 && g.measureText(t + ell).width > maxW) t = t.slice(0, t.length - 1);
  return t + ell;
}
function drawContain(g, img, x, y, w, h){
  ...
  var dx = x + (w - dw) / 2, dy = y + (h - dh) / 2;
  try { g.drawImage(img, dx, dy, dw, dh); } catch (eC) {}
}
function drawVisualShot(g, img, x, y, w, h){
  g.save();
  roundRectSafe(g, x, y, w, h, Math.min(w, h) * 0.03);
  g.clip();
  g.fillStyle = "#070910";
  g.fillRect(x, y, w, h);
  var pad = Math.min(w, h) * 0.045;
  drawContain(g, img, x + pad, y + pad, w - pad * 2, h - pad * 2);
  g.restore();
}
function visualKindLine(c){
  var bits = [];
  if (c && c.y) bits.push(String(c.y));
  var k = kindLabel(c);
  if (k) bits.push(k);
  return bits.join(" · ");
}
async function loadComposeImg(card){
  var routes = [], seen = {};
  function add(u){ if (u && !seen[u]) { seen[u] = 1; routes.push(u); } }
  if (card && card.img) {
    var big = wikiSrcList(card.img, 1200);
    var sm = wikiSrcList(card.img, 500);
    for (var i = 0; i < big.length; i++) add(big[i]);
    for (var j = 0; j < sm.length; j++) add(sm[j]);
  }
  add(imgUrl(card.i));
  add(imgSmall(card.i));
  var img = null;
  for (var ri = 0; ri < routes.length; ri++) {
    try {
      ...
      if (img && (img.naturalWidth || img.width) > 2) break;
    } catch (eL) { img = null; }
  }
  return img;
}
async function finishComposeBlob(cv, W, H, gen, L, missingArt, scale){
  ...
  if (allArtMissing) {
    setStatus("None of the " + tray.length + " images loaded, so there is nothing to post — the frame came out empty. The art host is unreachable from this connection.", true);
    el("dl").hidden = true; el("copy").hidden = true; el("share").hidden = true;
    el("blankwarn").hidden = true; el("retryscale").hidden = true;
    try { window.__lastComposeOk = false; } catch (eA) {}
    return;
  }
  el("dl").hidden = false;
  el("copy").hidden = false;
  if (navigator.canShare && navigator.canShare({ files: [new File([""], "t.png", { type: "image/png" })] })) el("share").hidden = false;
  if (missingArt.length) setStatus("Made it, but " + missingArt.length + " image" + (missingArt.length > 1 ? "s" : "") + " would not load: " + missingArt.join(", ") + ".", true);
  ...
  try { await showSaveableBlob(blob); }
    ...
    try { showSaveable(cv.toDataURL("image/png")); } catch (eS2) {}
  }
  try { window.__lastComposeOk = true; } catch (eOk) {}
  try { tutComposed(true); } catch (eT) {}
  try { setTimeout(fbMaybeAskQuestionnaire, 2500); } catch (eF) {}
}
async function composeVisual(gen){
  setStatus("composing…");
  const missingArt = [];
  const loaded = [];
  for (var i = 0; i < tray.length; i++) {
    var img = await loadComposeImg(tray[i]);
    if (gen !== composeGen) return;
    if (!img) missingArt.push(tray[i].n);
    loaded.push({ card: tray[i], img: img });
  }
  const PAD = 80, FOOT = 88, TITLE = 58, SUB = 30, GAP = 56;
  var shots = [];
  for (var si = 0; si < loaded.length; si++) {
    var im = loaded[si].img;
    var iw = im ? (im.naturalWidth || im.width || 1) : 745;
    var ih = im ? (im.naturalHeight || im.height || 1) : 1040;
    shots.push({ card: loaded[si].card, img: im, iw: iw, ih: ih, ar: iw / ih });
  }
  var W, H, places = [];
  if (shots.length === 1) {
    var maxW = 1480, maxH = 980;
    var s1 = Math.min(maxW / shots[0].iw, maxH / shots[0].ih);
    if (s1 > 1.25) s1 = 1.25;
    var dw = Math.round(shots[0].iw * s1), dh = Math.round(shots[0].ih * s1);
    W = Math.max(dw + PAD * 2, 1080);
    H = PAD + dh + 28 + TITLE + SUB + FOOT;
    places.push({ x: Math.round((W - dw) / 2), y: PAD, w: dw, h: dh });
  } else {
    var SLOT_H = shots.length <= 3 ? 640 : 520;
    var row = [];
    var rows = [];
    var rowW = 0;
    var MAX_ROW = 2400;
    for (var k = 0; k < shots.length; k++) {
      var sw = Math.round(SLOT_H * shots[k].ar);
      if (sw < 260) sw = 260;
      if (sw > 900) sw = 900;
      var add = sw + (row.length ? GAP : 0);
      if (row.length && rowW + add > MAX_ROW) {
        rows.push({ items: row, w: rowW });
        row = [];
        rowW = 0;
        add = sw;
      }
      row.push({ i: k, w: sw, h: SLOT_H });
      rowW += add;
    }
    if (row.length) rows.push({ items: row, w: rowW });
    var gridW = 0;
    for (var ri = 0; ri < rows.length; ri++) if (rows[ri].w > gridW) gridW = rows[ri].w;
    W = Math.max(gridW + PAD * 2, 1080);
    H = PAD + rows.length * (SLOT_H + 28) + (rows.length - 1) * GAP + TITLE + SUB + FOOT;
    var y0 = PAD;
    for (var rj = 0; rj < rows.length; rj++) {
      var x0 = Math.round((W - rows[rj].w) / 2);
      for (var cj = 0; cj < rows[rj].items.length; cj++) {
        var it = rows[rj].items[cj];
        places[it.i] = { x: x0, y: y0, w: it.w, h: it.h };
        x0 += it.w + GAP;
      }
      y0 += SLOT_H + 28 + GAP;
    }
  }
  const Lvis = { name: shots.length === 1 ? "the single" : "the shelf", cols: shots.length, rows: 1, W: W, H: H };
  const MAX_PX = 8000000;
  const autoScale = (W * H > MAX_PX) ? Math.sqrt(MAX_PX / (W * H)) : 1;
  ...
  if (!g) {
    reportBlank(Lvis, W, H, cv, null, "the browser would not give this page a 2D canvas context at " + cv.width + "x" + cv.height + ".");
    return;
  }
  ...
  try {
    for (var di = 0; di < shots.length; di++) {
      var p = places[di];
      if (!p) continue;
      if (shots[di].img) drawVisualShot(g, shots[di].img, p.x, p.y, p.w, p.h);
      if (shots.length > 1) {
        g.fillStyle = "#8a93a8";
        g.font = "600 22px system-ui,sans-serif";
        g.textAlign = "center";
        g.fillText(fitLine(g, shots[di].card.n, p.w), p.x + p.w / 2, p.y + p.h + 22);
      }
    }
    var title = el("label").value.trim() || (shots[0] ? shots[0].card.n : "");
    var sub = shots.length === 1 ? visualKindLine(shots[0].card) : "";
    var maxLabel = W - PAD * 2;
    g.fillStyle = "#f4f5f8";
    g.font = "800 44px system-ui,sans-serif";
    g.textAlign = "center";
    g.fillText(fitLine(g, title, maxLabel), W / 2, H - FOOT - SUB - 8);
    if (sub) {
      g.fillStyle = "#8a93a8";
      g.font = "22px ui-monospace,monospace";
      g.fillText(fitLine(g, sub, maxLabel), W / 2, H - FOOT + 18);
    }
    g.save(); g.globalAlpha = 0.16; g.fillStyle = "#fff"; g.font = "800 40px system-ui,sans-serif"; g.textAlign = "center";
    g.save(); g.translate(W * 0.27, PAD + 80); g.rotate(-Math.PI / 9); g.fillText("catchemtcg.com", 0, 0); g.restore();
    g.restore();
    g.fillStyle = "#36d399"; g.font = "800 40px system-ui,sans-serif"; g.textAlign = "left";
    g.fillText("Catch'em", PAD, H - 36);
    await finishComposeBlob(cv, W, H, gen, Lvis, missingArt, scale);
  } catch (eV) {
    setStatus("could not compose: " + (eV.message || "unknown"), true);
    try { window.__lastComposeOk = false; } catch (eV2) {}
  }
}
...
async function composeImage(){
  ...
  if (tray.length && tray.every(function(c){ return isVisual(c); })) {
    await composeVisual(gen);
    return;
  }
  setStatus("composing…");
  const missingArt = [];
  ...
  const CAP = connecting || (typeof artFlipActive === "function" && artFlipActive()) || (typeof artJustActive === "function" && artJustActive()) ? 0 : (tray.length <= 4 ? 70 : 0);
  ...
  const slotList = (typeof composeSlots === "function") ? composeSlots() : tray.map(function(c, idx){ return { card: c, art: false, trayIndex: idx }; });
  const ROWS = L.rows || Math.ceil(slotList.length / L.cols);
  ...
  try{
    ...
    for (let i=0;i<slotList.length;i++){
      var card = slotList[i].card;
      var img = null;
      if (Object.prototype.hasOwnProperty.call(loadedImg, card.i)) img = loadedImg[card.i];
      else {
        const u = imgUrl(card.i);
        ...
        loadedImg[card.i] = img || null;
        if (!img) missingArt.push(card.n);
      }
      if (!img) continue;
      if (gen !== composeGen) return;
      ...
      if (slotList[i].art) drawArtClip(g, img, x, y, CW, CH, card);
      else drawSlab(g, img, x, y, CW, CH, card);
      if (CAP && !slotList[i].art){ g.fillStyle="#8a93a8"; g.font="28px system-ui,sans-serif"; g.textAlign="center";
        g.fillText(card.n + " · " + card.y, x+CW/2, y+CH+46); }
    }
    ...
    g.fillText(fitLine(g, artists || "artist not recorded", W/2 - PAD), W-PAD, H-40);
