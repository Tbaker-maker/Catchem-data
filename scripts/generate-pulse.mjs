...
${der?.eraIndexes?.length?`<h2>\ud83c\udfdb Generation indexes</h2>${der.eraIndexes.map(e=>`<div class="row"><span><b>${e.era}</b><em> ${e.products} products \u00b7 ${e.avgGapPct!=null?`asking ${Math.abs(e.avgGapPct)}% ${e.avgGapPct>=0?"more":"less"} than TCGplayer \u00b7 `:"eBay-native era \u00b7 "}${e.listingsPerProduct} listings each</em></span><span class="mono">$${e.level.toLocaleString("en-US")}</span></div>`).join("")}${der?.supplyShifts?.length?`<h2>\ud83c\udf0a Supply shifts</h2>${der.supplyShifts.slice(0,5).map(x=>`<div class="row"><span><b>${x.name}</b><em> ${x.prev}\u2192${x.listings} listings${x.priceDPct!=null?` \u00b7 price ${x.priceDPct>0?"+":""}${x.priceDPct}%`:""} \u00b7 ${x.read}${x.catalystMatch?` \u00b7 ${x.catalystMatch}`:""}</em></span><span class="mono" style="color:${x.dPct>0?"var(--gold)":"var(--green)"}">${x.dPct>0?"+":""}${x.dPct}%</span></div>`).join("")}<div class="foot">Shelf count vs yesterday \u00b7 cause candidates, never verdicts \u00b7 gated at 20+ listings so small shelves can\u2019t fake big percents.</div>`:""}
<div class="foot">Era level = median tracked product. Baseline 100 today \u2014 momentum lines grow from tomorrow.</div>`:""}
