...
const FLEET = [
  ...
  { id: "live-page-smoke", script: "live-page-smoke.mjs", blocking: true,
    does: "Opens the PUBLISHED Pages URL in a real browser and asserts a fresh visitor sees the tutorial, that card images resolve to real bytes, that a conceptual query reaches a relation, and that 'charmander evolution' loads the actual line. BLOCKING because every other guard here reads the generator or the index: on 2026-08-26 three launch blockers passed the entire suite and were caught only by loading this URL in a fresh window. It fails loudly when no browser is present rather than skipping, because a smoke test that skips is a green tick over an untested page." },
  { id: "reply-line-guard", script: "reply-line-guard.mjs", blocking: true,
    does: "Proves no script drafts, suggests or queues reply TEXT, and that the research brief still states the rule. BLOCKING because Tyler's reply practice is the strongest asset this account has and it works because a person is actually typing - the moment a drafted reply exists in a queue it gets used on a tired evening, and the property that made it work is gone immediately rather than gradually. Surfacing WHERE a conversation is happening is allowed; the words are his." },
  ...
  { id: "response-audit", script: "response-audit.mjs", blocking: true,
    does: "Checks that every external call reads what the provider told it about its own completeness - HTTP status, Anthropic stop_reason, eBay total, X errors[]. BLOCKING because discarding one of those fields is how the release radar stayed frozen for eight days behind eight green runs: the API reported max_tokens every time and the script dropped the field. A call may ignore a signal, but only in writing, with a reason." },
  { id: "artifact-freshness", script: "artifact-freshness.mjs", blocking: true,
    does: "Takes every file a scheduled job promises to commit and fails if any is more than TWO runs stale. BLOCKING as of 2026-08-26, and the reason is WHERE THE SIGNAL LANDS rather than how loud it is: six consecutive red price runs sat in the GitHub Actions tab for three days, perfectly visible, in a place nobody on this project opens. Tyler checks the fleet. Paths that genuinely move on events rather than on a schedule are excused BY NAME with a reason, which is a decision somebody makes rather than a silence." },
  ...
  { id: "notice-eye", script: "notice-eye.mjs", blocking: true,
    does: "Loads the shipped editor and proves NOTICE observations name at least three distinct printed fields — not only attack names. BLOCKING because the eye tower reading three facts against forty is a product failure, and nothing else looks at those lines." },
  { id: "connecting-art-audit", script: "connecting-art-audit.mjs", blocking: true,
    does: "Decides left-right vs top-bottom vs grid for every complete combined illustration from the printed edges, not the wiki table. BLOCKING because stacking Carvanha on Sharpedo is visibly broken art, and one wrong panorama is a user who does not come back." },
  { id: "editor-hostile", script: "editor-hostile.mjs", blocking: true,
    does: "Loads the generated page and tries to break it: merge SyntaxErrors, empty/garbage asks, count nouns, evolution families, Another/Back, glance-test furniture, duplicate adds. BLOCKING because three guards went green while lineOptions could not even parse." },
