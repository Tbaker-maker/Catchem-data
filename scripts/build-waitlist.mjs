// build-waitlist.mjs — the landing page for a traffic spike.
//
// Tyler, 2026-08-23: "Create me a waitlist, this post is going viral. One page
// feature list thing that converts this volume. Under 16 hours and already 11.1k
// impressions."
//
// 11.1k against a previous best of 791 is fourteen times, and it is still
// climbing. The page has one job and roughly four seconds to do it.
//
// THREE DECISIONS THAT DRIVE EVERYTHING HERE:
//
...
const themes = JSON.parse(await readFile(join(ROOT, "data/themes.json"), "utf-8"));
const n = (x) => x.toLocaleString("en-US");
const CARDS = n(idx.length), PRICED = n(idx.filter(c => c.p != null).length),
      ARTISTS = n(new Set(idx.map(c => c.a).filter(Boolean)).size),
      SETS = n(new Set(idx.map(c => c.s)).size);
...
<meta property="og:image" content="https://catchemtcg.com/og.png">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta name="twitter:image" content="https://catchemtcg.com/og.png">
<meta property="og:url" content="https://catchemtcg.com">
<meta property="og:type" content="website">
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Crect width='32' height='32' rx='7' fill='%230a0c12'/%3E%3Ctext x='16' y='23' font-family='system-ui,sans-serif' font-size='19' font-weight='800' fill='%2336d399' text-anchor='middle'%3EC%3C/text%3E%3C/svg%3E">
<style>
...
:root{--ink:#0a0c12;--panel:#11141c;--raise:#171b25;--line:#20252f;--text:#e8ebf2;
  --soft:#8a93a6;--faint:#79839a;--live:#36d399;--ease:cubic-bezier(.22,.61,.36,1)}
...
/* HERO. Four seconds. The name, what it is, and a button — nothing above the
   fold that is not one of those three. */
...
.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--line);
  border:1px solid var(--line);border-radius:14px;overflow:hidden;margin:36px 0}
...
/* ??? reads as withheld rather than unfinished — the difference is whether it
   looks like a decision or an oversight. */
.feat h3 .tag:only-of-type{letter-spacing:.2em}

/* The form is the SECOND ask, below the tool. */
.form{background:linear-gradient(180deg,rgba(54,211,153,.05),transparent),var(--panel);
...
<div class="hero">
  ...
  <a class="cta" href="#join">Get on the list
    <span class="small">Early access when the rest of it opens</span></a>
  <p class="note">The post editor is in early access. Binder, streaks and sealed data are still being finished.</p>
</div>
...
<div class="form" id="join">
  ...
  <p class="sub">One email when the rest opens, and nothing else. <b>The tools stay free.</b> Automated posting will be paid, because it costs real money every time it runs — everything else does not.</p>
...
<script>
// Submit without leaving the page. A redirect to a Formspree confirmation screen
// loses somebody who arrived from a timeline and will not come back.
document.getElementById("wl").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // If the fetch fails, fall back to a normal submit rather than swallowing
    // the signup — a lost email from a spike is not recoverable.
    f.submit();
  }
});
</script>
...
await writeFile(join(ROOT, "research/assets/index-landing.html"), html);
console.log(`✓ waitlist: ${CARDS} cards claimed, 5 features marked LIVE, 2 marked IN DEVELOPMENT`);
