...
const MUST_RUN = [
  ...
  { script: "competence-guard.mjs", why: "every specialist agent declares principles, failure modes, blind spots and a recheck date" },
  ...
  { script: "doc-numbers.mjs", why: "a handover that lies is worse than none — it is the first thing the next chat reads, and four documents shipped with every count wrong" },
  { script: "crop-guard.mjs", why: "every user is on X and a clipped top is not an edge case, it is the only case — designing to the 1.25 crop line means failing at it" },
  ...
  { script: "notice-eye.mjs", why: "NOTICE observations must name at least three printed fields — attack-only is the eye tower reading 5% of the catalogue" },
  { script: "connecting-art-audit.mjs", why: "combined illustrations must lay out from the picture — the wiki table stacked Carvanha on Sharpedo and the art does not" },
  { script: "live-page-smoke.mjs", why: "the published editor must show a tutorial and painted images — three launch blockers passed every other guard and were only found by opening the live URL", offPipeline: true },
  ...
  { script: "prompt-audit.mjs", why: "56 prompts a real person would type, across ten intents — not the eleven I invented" },
  ...
  { script: "editor-copy-rules.mjs", why: "every string the editor GENERATES goes onto a public timeline and nothing checked it — the rules our other surfaces obey now apply here too" },
  { script: "editor-claim-match.mjs", why: "an idea's words must match the cards it shows — the Koga bug shipped a true fact beside the wrong card, which is the failure that reads as researched" },
  { script: "editor-money-credit.mjs", why: "a wrong price goes to a card show and a missing credit goes on a public image — the two failures that cost a user something real" },
